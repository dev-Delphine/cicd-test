...
```
dbx execute --cluster-name=<name of interactive cluster> --job=cicd-test-sample-integration-test
```

For a test on an automated job cluster, deploy the job files and then launch:
```
dbx deploy --jobs=cicd-test-sample-integration-test --files-only
dbx launch --job=cicd-test-sample-integration-test --as-run-submit --trace
```
...
- To trigger the release pipeline, get the current version from the `cicd_test/__init__.py` file and tag the current code version:
```
